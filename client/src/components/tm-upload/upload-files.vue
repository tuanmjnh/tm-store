<template>
  <div class="file-upload">
    <input v-if="reload" type="file" id="file-upload" :multiple="multiple===true?true:false" @change="onChange($event)"
      :accept="extension" />
      ...
      Array.from(Array(event.target.files.length).keys()).map(x => {
        if (this.multiple) {
          tmp = {
            ...
            fileName: this.autoName === true ? NewGuid() + event.target.files[x].name.getExt() : event.target.files[x].name,
