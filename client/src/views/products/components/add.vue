  ...
  <q-card style="min-width:800px">
    <q-toolbar>
      ...
      <q-btn flat type="submit" :dense="$store.getters.dense.button" color="blue" icon="check_circle"
        :label="$t('global.add')" :loading="$store.state.loading.get||$store.state.loading.post"
        @click.prevent="onSubmit">
      </q-btn>
      <q-btn flat round dense :color="$store.state.app.darkMode?'':'grey-7'"
        :icon="maximized?'fullscreen_exit':'fullscreen'" :disable="$store.state.loading.get||$store.state.loading.post"
