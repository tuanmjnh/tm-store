<template>
  <q-card style="min-width:800px">
    <q-toolbar>
      ...
      <q-btn v-if="items&&items.length" flat round dense color="primary" icon="done" :disable="loading"
        @click="onFinish">
    ...
    <q-separator />
    <q-table v-if="items&&items.length" :data="items" :columns="columns" row-key="_id" :visible-columns="visibleColumns"
      :loading="loading" :dense="$store.getters.dense.table" :no-data-label="$t('table.no_data')"
      :no-results-label="$t('table.no_filter_data')" :rows-per-page-label="$t('table.row_per_page')"
      :rows-per-page-options="[10, 20, 50 ,100, 200, 0]" :pagination.sync="pagination" :filter="pagination.filter"
      binary-state-sort>
      <template v-slot:top>
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="quantity_import" :props="props">
          ...
          <q-td key="price_import" :props="props">
            {{ props.row.price_import|NumberFormat($store.getters.language) }}
          ...
          <q-td auto-width key="status" :props="props">
            <i v-if="props.row.status" class="material-icons text-green">
              cloud_done
              <q-tooltip v-if="!$q.platform.is.mobile">{{$t('product.status_in')}}</q-tooltip>
            </i>
            <i v-else class="material-icons text-orange">
              cloud_off
              <q-tooltip v-if="!$q.platform.is.mobile">{{$t('product.status_out')}}</q-tooltip>
            </i>
          </q-td>
        </q-tr>
      </template>
    ...
    <q-card-section class="text-center" v-else>
      <tm-load-files :loading.sync="loading" :button="true" :label="$t('files.open_file')"
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { name: 'quantity_import', field: 'quantity_import', label: 'product.quantity_import', align: 'center', sortable: true, required: true },
        { name: 'price_import', field: 'price_import', label: 'product.price_import', align: 'center', sortable: true, required: true },
        { name: 'price', field: 'price', label: 'product.price_sale', align: 'center', sortable: true },
        { name: 'price_discount', field: 'price_discount', label: 'product.price_discount', align: 'center', sortable: true },
