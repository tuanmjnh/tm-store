...
const onSetLanguage = (item) => {
  if (storeApp.language !== item) {
    storeApp.setLanguage(`${item.cc_iso}-${item.cc}`)
    isShowLanguage.value = !isShowLanguage.value
  }
}
const onToggleDarkMode = () => {
  storeApp.setDarkMode()
}
...
<template>
  <div class="bg-gradient">
    <van-form @submit="onSubmit" class="van-form-square-top">
      <div class="form-header">
        ...
        <div class="right">
          <van-space>
            <span :class="`fi fi-${storeApp.language}`" @click="isShowLanguage = !isShowLanguage" />
  ...
  <van-action-sheet v-model:show="isShowLanguage" :cancel-text="$t('global.cancel')"
    :description="$t('navbar.switchLanguage')" close-on-click-action @select="onSetLanguage">
    <van-cell-group inset>
      <van-cell v-for="(e, i) in languages" :key="i" @click="onSetLanguage(e)">
        ...
        <template #right-icon v-if="`${e.cc_iso}-${e.cc}` === storeApp.language">
